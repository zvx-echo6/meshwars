...
- **Peak Tagger** — most summits visited. A square credits a summit when it is within 1.5 km horizontally **and** within 300 m of that summit's own elevation, and each square is assigned to its single nearest qualifying summit. Both halves are load-bearing. Horizontal distance alone is the wrong axis: at 8 km, 11 of the 17 Wasatch summits become claimable from a valley street 6,000 ft below, while the same 8 km still reaches only 40 of 4,851 summits statewide. And without the nearest-summit rule one hike would credit up to 18 peaks at once, since summits cluster — worse still, the runtime non-stacking rule resolves a square to its *dearest* place, which for summits means the taller neighbour. Before this (until 2026-08-31) a summit was the single square containing its peak, and in the game's whole history nobody had ever tagged one. The mapping is precomputed by `scripts/build_summit_cells.py` against the planet DEM and ships as `app/reference/summit_cells.csv`, because the app hosts have no elevation data
