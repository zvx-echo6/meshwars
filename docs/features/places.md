...
The anchors come from two US Census 2024 Gazetteer files, built by `scripts/build_places_csv.py` (fetch-place, fetch-ua, build): the PLACE file (incorporated cities/towns and Census Designated Places, no type filter — an unincorporated CDP is as valid an anchor as an incorporated city) and, since 2026-09-07, the URBAN AREA file. Place rows alone left real gaps: San Francisco's corporate limits legally include the Farallon Islands, a non-contiguous exclave ~48km offshore, so Census's one interior point for that whole shape lands out at sea rather than on the mainland, and the Las Vegas Strip sits in unincorporated Clark County (Winchester/Paradise CDPs), outside the City of Las Vegas's own boundary and short of neighboring anchors' circles too — both cities read as "remote" despite sitting in the middle of a metro area. Urban areas are drawn from contiguous population density rather than jurisdiction lines, so one urban-area anchor per metro closes gaps like these generally instead of hand-patching individual cities. Both files are filtered to the same play-area-plus-1-degree bbox; see the script's module docstring for the full mechanism and `tests/test_places_anchors.py` for the regression coverage.
