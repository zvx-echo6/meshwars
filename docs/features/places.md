...
Explorer originally became most Explorer points that month, instead of most squares nobody had ever claimed. That did not last: the 100-point weekly cap meant everyone who played seriously finished a month within the same narrow band, so the points version separated nobody either — the same flaw that retired Most Consistent (see "Decisions reversed" in `docs/features/long-season.md`). Explorer is now retired outright, replaced by three awards that count VISITS rather than points — Tourist (landmarks), Park Hopper (parks), Peak Tagger (summits) — one row in `place_activation` of the matching type inside the month is one visit, and the place's point value is never read. The weekly cap still does the work: it makes the three mutually exclusive in practice, since a single remote summit spends the whole week, so each one describes a different way to play rather than a different view of the same grind. Frontier is unaffected by any of this — it kept counting squares beyond city limits and dropped its virgin-ground restriction (which only existed so Frontier would be a strict subset of the old squares-based Explorer) before Explorer was ever redefined around points.

**Built so far:** a place credit (`place_activation`) adds to the team total (`app/mc_scoring.team_place_points`, folded into `team_totals()`) and to a personal running figure (`explorer_points` on each player row in `/api/v1/players`), both scoped to the season by `awarded_at` falling inside it — this running figure is unrelated to the monthly awards above and is untouched by any of their redefinitions. `app/results.py`'s monthly awards are built: Tourist, Park Hopper, and Peak Tagger each count that type's visits inside the month, and Frontier keeps its own out-of-town square count.
