...
- 100 points per person per week, whatever the mix. A place only credits if its FULL value still fits in what is left of the cap — no partial credit. A place that does not fit is turned away whole; it is not consumed by the attempt and is worth its full value again next week. Other squares carrying cheaper places can still land in the same week — the cap is spent square by square, not closed by the first refusal.
- Places do NOT stack. One square can map to several places — a landmark standing inside a big park is the ordinary case — and only the HIGHEST-VALUE live one credits. The lesser place is dropped outright: it does not pay alongside the winner, and it is not a fallback if the winner is over budget or was already credited this week (in which case that square simply pays nothing this week). Equal point values are broken by a stable hash of the place id (`app/place_scoring._stable_tiebreak`), so the same square always resolves to the same winner and insertion order never decides it. Stacking was the behaviour until 2026-08-27 and was never intended — one ping paid twice for one errand.
