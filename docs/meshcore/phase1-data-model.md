...
1. **A player can switch their own team, once per calendar month.** This reverses the original decision recorded here, which was that a player's team was fixed at registration and any change had to be an administrative action on their behalf. The original worry — that switching sides mid-season lets someone hand tiles to whichever team they defect to — turned out to have a narrower fix than banning self-service switching outright: ground stays put. A tile's `owner_team` is frozen at paint time and is never re-derived from `player.team`, so a player who switches takes nothing they painted with them; the squares they held stay with the team that held them. What does travel to the new team is check-in points, exploration points, and streaks, because those are already computed live off `player.team` rather than stored per-tile. With the actual defection risk closed off at the ground level, the only thing left to bound was how often someone could do it, so a player is capped at one self-service switch per calendar month (the same calendar month the scoring month uses). An operator can still switch any player's team on their behalf, unlimited, exactly as this decision originally anticipated — that path never went away, it just stopped being the only one.
